...
// @route   POST /api/auth/phone/verify-token
// @desc    Verify Firebase ID Token and login/register
// @access  Public
router.post('/phone/verify-token', [
  ...
  if (!firebaseAdmin) {
    return res.status(500).json({ status: 'error', message: 'Firebase is not configured on the server' });
  }
  ...
  try {
    ...
    if (!phone) {
      return res.status(400).json({ status: 'error', message: 'ID Token does not contain a phone number' });
    }

    // Convert +91... to 10-digit if necessary (assuming Indian numbers for now as per previous context)
    // Or keep the full international format. The User model expects 10 digits.
    ...
    if (user) {
      ...
      await logEvent({
        userId: user._id,
        action: 'LOGIN_PHONE_FIREBASE',
        details: { message: 'User logged in via Firebase Phone Auth' },
        req
      });
      ...
      // New user, return success but flag that they need to complete registration
      return res.json({
        status: 'success',
...
// @desc    Register with phone and name (after OTP verification)
// @access  Public
router.post('/phone/register', [
  body('phone').matches(/^[0-9]{10}$/).withMessage('Valid phone required'),
  body('name').trim().notEmpty().withMessage('Name is required'),
  body('email').isEmail().withMessage('Valid email required'),
  body('password').isLength({ min: 8 }).withMessage('Strong password required')
], asyncHandler(async (req, res) => {
  const { phone, name, email, password } = req.body;

  let user = await User.findOne({ phone });
  if (!user) {
    // If user doesn't exist but has been verified via Firebase, created it now
    user = new User({
      phone,
  ...
  if (!user.isPhoneVerified) {
    return res.status(400).json({ status: 'error', message: 'Phone must be verified first' });
  }

  // Check if email already taken
  const emailExits = await User.findOne({ email, _id: { $ne: user._id } });
  if (emailExits) {
    return res.status(400).json({ status: 'error', message: 'Email already registered' });
  }

  // Update the temporary user created during OTP request
  user.name = name;
  user.email = email;
