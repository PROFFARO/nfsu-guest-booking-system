                        ...
                        <div key={i} className="p-3 border border-border rounded-sm bg-muted/20 text-[10px] space-y-2 font-noto-regular overflow-hidden shadow-xs">
                            ...
                            <div className="flex justify-between items-start">
                                ...
                                <div className="text-right">
                                    <span className="font-noto-bold text-xs text-[#1a365d] dark:text-blue-400">₹{room.price}</span>
        ...
        if (action === 'calculate_stay_quote' && result && !result.error) {
            return (
                <div className="mt-2 p-3 border border-blue-100 rounded-md bg-blue-50/20 text-[10px] space-y-2 font-noto-regular">
                    ...
                    <div className="bg-white dark:bg-slate-800/80 p-2 rounded border border-blue-100/50 space-y-2">
                        <div className="flex justify-between items-center">
                            <span className="text-muted-foreground">Stay for Room {result.roomNumber}</span>
                            <span className="font-noto-bold text-[#1a365d] dark:text-blue-400">₹{result.totalAmount}</span>
                        </div>
                ...
                <div className="mt-2 p-3 border border-border rounded-md bg-muted/10 text-[10px] space-y-2 font-noto-regular max-w-[240px]">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                            ...
                            <p className="text-[9px] text-[#1a365d] dark:text-blue-400 uppercase font-noto-bold opacity-80">{result.role}</p>
                        </div>
                    </div>
        ...
        if (action === 'modify_booking' && result && result.success) {
            return (
                <div className="mt-2 p-3 border border-emerald-100 rounded-md bg-emerald-50/20 text-[10px] space-y-2 font-noto-regular">
                    ...
                    <div className="grid grid-cols-2 gap-4 bg-white dark:bg-slate-800/50 p-2 rounded border border-emerald-100/50">
                        <div>
        ...
        if (action === 'get_my_gatepass' && result && result.success) {
            return (
                <div className="mt-2 p-3 border border-border rounded-lg bg-card space-y-3 font-noto-regular shadow-sm max-w-[280px]">
                    ...
                    <div className="flex justify-center py-2 bg-white dark:bg-slate-800 rounded-md">
                        <img
                            src={result.qrCode}
                            alt="Gatepass QR"
                            className="h-32 w-32"
                    ...
                    <div className="text-center space-y-1">
                        ...
                        <p className="text-xl font-noto-bold tracking-[0.3em] text-[#1a365d] dark:text-blue-400 uppercase">{result.token}</p>
                    </div>
        ...
        if (action === 'request_supplies' && result && result.success) {
            ...
            return (
                <div className="mt-2 p-3 border border-emerald-200 rounded-md bg-emerald-50/30 text-[10px] space-y-2 font-noto-regular">
                    ...
                    <div className="bg-white dark:bg-slate-800/60 rounded-md border border-emerald-100 divide-y divide-emerald-100">
                        {items.map((item, idx) => (
            ...
            return (
                <div className="mt-2 p-4 border border-border rounded-lg bg-card space-y-3 shadow-sm font-noto-regular">
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <span className="text-[#1a365d] dark:text-blue-400 font-noto-bold">₹{result.price}/night</span>
                    </div>
                    ...
                    {result.map((faq, i) => (
                        <div key={i} className="p-3 border border-border rounded-sm bg-sky-50/30 text-[10px] space-y-1 font-noto-regular shadow-xs">
                            <div className="flex items-start gap-2">
                                <Search className="h-3 w-3 mt-0.5 text-[#1a365d] dark:text-blue-400" />
                                <span className="font-noto-bold text-[#1a365d] dark:text-blue-400">{faq.question}</span>
                            </div>
        ...
        if (action === 'get_my_bookings' && Array.isArray(result)) {
            return (
                <div className="mt-3 space-y-3">
                    <p className="text-[10px] font-noto-bold text-[#1a365d] dark:text-blue-400 uppercase tracking-widest px-1">Active Bookings:</p>
                    {result.length === 0 ? (
            ...
            return (
                <div className="mt-2 p-3 border border-emerald-100 rounded-md bg-emerald-50/20 text-[10px] space-y-2 font-noto-regular">
                    ...
                    <div className="space-y-1 pl-1">
                        <p className="text-muted-foreground">Modified: <span className="text-[#1a365d] dark:text-blue-400 font-bold italic">{result.updatedFields?.join(', ') || 'dates'}</span></p>
                        {result.priceDiff !== 0 && (
                        ...
                        <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
                            {threads.length === 0 ? (
                                ...
                                threads.map(t => (
                                    ...
                                    >
                                        <MessageSquare className={`h-3.5 w-3.5 mt-0.5 shrink-0 ${threadId === t._id ? 'text-[#1a365d] dark:text-blue-400' : 'text-muted-foreground/50'}`} />
                                        <div className="flex-1 min-w-0 pr-6">
                                            <p className={`text-[10px] truncate font-noto-bold uppercase tracking-tight ${threadId === t._id ? 'text-[#1a365d] dark:text-blue-400' : 'text-foreground/80'}`}>
                        ...
                        <div className="flex flex-col items-center justify-center text-center py-4 px-2">
                            ...
                            {!threadId && (
                                <div className="w-full max-w-[420px] space-y-2.5">
                                    ...
                                    {/* Rooms & Lodging */}
                                    <div className="space-y-1">
                                        <p className="text-[7px] font-noto-bold text-[#1a365d] dark:text-blue-400 uppercase tracking-wider pl-0.5 opacity-60">Rooms & Lodging</p>
                                    ...
                                    {/* Reservations */}
                                    <div className="space-y-1">
                                        <p className="text-[7px] font-noto-bold text-[#1a365d] dark:text-blue-400 uppercase tracking-wider pl-0.5 opacity-60">Reservations</p>
                                        <div className="grid grid-cols-3 gap-1">
                                            {[
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="grid grid-cols-2 gap-1">
                                            {[
                                                ...
                                                <button key={i} onClick={() => handleSendWithContent(task.query)}
                                                    className="p-1.5 text-[8px] font-noto-bold text-[#1a365d] dark:text-blue-400 bg-white dark:bg-slate-800 hover:bg-amber-500 hover:text-white border border-amber-500/15 rounded-sm transition-all text-left truncate">
                                                    {task.label}
                                                </button>
                                            ))}
                                        </div>
                                    ...
                                    {/* Services & Support */}
                                    <div className="space-y-1">
                                        <p className="text-[7px] font-noto-bold text-[#1a365d] dark:text-blue-400 uppercase tracking-wider pl-0.5 opacity-60">Services & Support</p>
                                        <div className="grid grid-cols-2 gap-1">
                                            {[
                                    ...
                                    <div className="space-y-1 pb-2">
                                        <p className="text-[7px] font-noto-bold text-[#1a365d] dark:text-blue-400 uppercase tracking-wider pl-0.5 opacity-60">Information</p>
                                        <div className="grid grid-cols-3 gap-1">
                                            {[
            ...
            <div className="flex justify-between items-start mb-2">
                <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                        <span className="text-[11px] font-noto-bold text-[#1a365d] dark:text-blue-400 uppercase tracking-tight">Room {booking.room}</span>
                ...
                <div className="text-right">
                    <div className="text-[11px] font-noto-bold text-[#1a365d] dark:text-blue-400">₹{booking.total}</div>
    ...
    return (
        <div className="p-3 space-y-3 font-noto-regular">
            <div className="grid grid-cols-2 gap-2.5">
                <div className="space-y-1">
                    <label className="text-[8px] font-noto-bold text-muted-foreground uppercase flex items-center gap-1"><Calendar className="h-2.5 w-2.5 text-[#1a365d] dark:text-blue-400" /> Check-In</label>
                    <input
                        type="date"
                        name="newCheckIn"
                ...
                <div className="space-y-1">
                    <label className="text-[8px] font-noto-bold text-muted-foreground uppercase flex items-center gap-1"><Calendar className="h-2.5 w-2.5 text-[#1a365d] dark:text-blue-400" /> Check-Out</label>
                    <input
                        type="date"
                        name="newCheckOut"
            ...
            <div className="space-y-1">
                <label className="text-[8px] font-noto-bold text-muted-foreground uppercase flex items-center gap-1"><User2 className="h-2.5 w-2.5 text-[#1a365d] dark:text-blue-400" /> Guest Name</label>
                <input
                    type="text"
                    name="guestName"
            ...
            <div className="grid grid-cols-2 gap-2.5">
                <div className="space-y-1">
                    <label className="text-[8px] font-noto-bold text-muted-foreground uppercase flex items-center gap-1"><Briefcase className="h-2.5 w-2.5 text-[#1a365d] dark:text-blue-400" /> Purpose</label>
                    <select
                        name="purpose"
                ...
                <div className="space-y-1">
                    <label className="text-[8px] font-noto-bold text-muted-foreground uppercase flex items-center gap-1"><Tag className="h-2.5 w-2.5 text-[#1a365d] dark:text-blue-400" /> Guests</label>
                    <input
                        type="number"
                        name="numberOfGuests"
                        min="1"
...
function BookingSelector({ bookings, onConfirm }) {
    ...
    return (
        <div className="mt-3 space-y-2 border-t border-border pt-3">
            <p className="text-[9px] font-noto-bold text-[#1a365d] dark:text-blue-400 uppercase tracking-widest mb-2">Select Bookings to Process:</p>
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-start">
                                ...
                                <span className="text-[9px] font-noto-bold text-[#1a365d] dark:text-blue-400">₹{b.total}</span>
                            </div>
